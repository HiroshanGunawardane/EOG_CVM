function [xhat, SNRr, SNR, RMSE] = ConstAcc(R, Q, inp, show_plot, event)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%modif
%  inp = inp';
%  inp = detrend(inp);
...
% kfinal = length(w0);
M(:,:,1) = eye(3); % Error covariance initial value
for k = 1:kfinal
    %
    ...
    M(:,:,k+1) = [1 k (k^2)/2;0 1 k;0 0 1]*P(:,:,k)*[1 k (k^2)/2;0 1 k;0 0 1]'+Bwd*R*Bwd';
    %
    %   Kalman gain
    %
    K(:,:,k)=P(:,:,k)*C'/Q;
...
xhat_t(:,:,1) = [0 0 0]'; % initial estimate
y_a = input_x(1,1:kfinal)';

for k = 1:kfinal
    xhat_m(:,: ,k) = xhat_t(:,:,k)+K(:,:,k)*...
        (y_a(k)-Cd*xhat_t(:,:,k)); % xhat(k|k) (measurement update)
    xhat_t(:,:,k+1) = [1 k (k^2)/2;0 1 k;0 0 1]*xhat_m(:,:,k);%+Bd*u(k); % xhat(k+1|k)
end
...
if show_plot
    ...
    title('KF with Constant Acc. Model'); 
    legend('raw EOG','KF-Constant Acc. - EOG')
end
