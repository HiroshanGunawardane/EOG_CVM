...
filternames = struct("KFBrownian", "Brownian", "KFConstVel", "Constant Velocity", ...
    "ConstAcc", "Constant Acceleration", "Bandpass", "Bandpass");
participant = fname{1};
...
for row = 1:size(data, 1)
    % Format as follows [name Q calibration_factor [data]]
    fieldname = data(row,1);
%     while isfield(filtered_eog, fieldname)
%         fieldname = fieldname + "1";
%     end
    
    filtered_eog.(fieldname) = double(data(row, 4:end))*double(data(row, 3));
    Q_values.(fieldname) = data(row, 2);
end
