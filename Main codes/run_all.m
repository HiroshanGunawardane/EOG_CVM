...
% Brownian: Q = 0.01
% Const vel, Const acc: Q = 0.5
filter_types = {@Bandpass @KFBrownian @KFConstVel @ConstAcc ... 
    @KFWesthInputAll, @LinearRecipoAll};
filter = filter_types{4};
Q = 0.5;

excel_fname = "ConstAcc.xlsx";
plot_folder_name = "ConstAcc";
write_excel = true;
save_plots = true;
save_csv = true;

run_all_participants = true;
participants_to_run = ["P26"];
%-------------------------------
main_path = uigetdir;
...
for i = 1:length(xdf_paths)
    ...
    [calibration_factor_eog, calibration_factor_elink, R_value, ...
        ...
        to_copy_outliers_removed, to_copy_removed_outliers_removed, SNR_array, ... 
        current_participant_toSave] = run_participant(xdf_path, outlier_iqr_scale, filter);
    
    all_participants.(participant) = current_participant_toSave; % Raw data
    
    SNR_matrix = [SNR_matrix; SNR_array];
...
%             ["Std" vecnorm(double(std_matrices_out.(condition)), 2, 1)/length(participant_array)];
    ...
    SNR_headers = ["C1" "C2" "C3" "C4"];
    SNR_table = [["" SNR_headers]; [participant_array' SNR_matrix]];
    ...
    xlswrite(excel_fname, SNR_table, "SNR", "A1");
